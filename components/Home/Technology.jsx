...
export default function Technology() {
  ...
  useEffect(() => {
    const query = `*[_type == "news" && "technology" in categories]{
  ...
  const categoryDisplay = (category) => {
    const displays = {
      technology: isUrdu ? "ٹیکنالوجی" : "Technology",
      innovation: isUrdu ? "انویسٹیشن" : "Innovation",
      software: isUrdu ? "سافٹ ویئر" : "Software",
      hardware: isUrdu ? "ہارڈ ویئر" : "Hardware",
    };
  ...
  return (
    <ScrollBasedAnimation direction="up" delay={0.2}>
      <section id="tech-news" className={`max-w-7xl mx-auto px-8 py-16 md:py-24 ${isUrdu ? "rtl" : "ltr"}`}>
        {/* Section Header */}
        <div className="flex items-center justify-between mb-12">
          <div className="flex items-center gap-4">
            <div className="w-1 h-8 bg-black" />
            <h2 className="text-3xl md:text-4xl font-bold tracking-tight uppercase">
              {isUrdu ? "ٹیکنالوجی خبریں" : "Technology"}
            </h2>
          </div>
          <a href="/tech-news" className="text-sm font-medium hover:underline transition">
