...
export default function PKNews() {
  const pathname = usePathname();
  const isUrdu = pathname.startsWith("/ur"); // Detect Urdu route
  const [news, setNews] = useState([]);

  useEffect(() => {
    const query = `*[_type == "news" && "pakistan" in categories]{
  ...
  const categoryDisplay = (category) => {
    const displays = {
      pakistan: isUrdu ? "پاکستان" : "Pakistan",
      politics: isUrdu ? "سیاست" : "Politics",
      national: isUrdu ? "ملک" : "National",
      economy: isUrdu ? "اقتصادیات" : "Economy",
      society: isUrdu ? "معاشرہ" : "Society",
    };
  ...
  return (
    <ScrollBasedAnimation direction="up" delay={0.2}>
      <section id="pk-news" className={`max-w-7xl mx-auto px-8 py-16 md:py-24 ${isUrdu ? "rtl" : "ltr"}`}>
        {/* Section Header */}
        <div className="flex items-center justify-between mb-12">
          <div className="flex items-center gap-4">
            <div className="w-1 h-8 bg-black" />
            <h2 className="text-3xl md:text-4xl font-bold tracking-tight uppercase">
              {isUrdu ? "پاکستانی خبریں" : "Pakistan News"}
            </h2>
          </div>
          <a href="/pk-news" className="text-sm font-medium hover:underline transition">
