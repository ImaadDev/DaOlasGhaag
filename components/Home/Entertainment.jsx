...
export default function Entertainment() {
  ...
  useEffect(() => {
    const query = `*[_type == "news" && "entertainment" in categories]{
  ...
  const categoryDisplay = (category) => {
    const displays = {
      entertainment: isUrdu ? "انٹرٹینمنٹ" : "Entertainment",
      showbiz: isUrdu ? "شوبز" : "Showbiz",
      movies: isUrdu ? "فلمیں" : "Movies",
      music: isUrdu ? "میوزک" : "Music",
    };
  ...
  return (
    <ScrollBasedAnimation direction="up" delay={0.2}>
      <section id="showbiz-news" className={`max-w-7xl mx-auto px-8 py-16 md:py-24 ${isUrdu ? "rtl" : "ltr"}`}>
        {/* Section Header */}
        <div className="flex items-center justify-between mb-12">
          <div className="flex items-center gap-4">
            <div className="w-1 h-8 bg-black" />
            <h2 className="text-3xl md:text-4xl font-bold tracking-tight uppercase">
              {isUrdu ? "انٹرٹینمنٹ خبریں" : "Entertainment"}
            </h2>
          </div>
          <a href="/showbiz-news" className="text-sm font-medium hover:underline transition">
