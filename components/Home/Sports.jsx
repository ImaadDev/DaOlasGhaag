...
import sanityClient from "@/lib/sanityClient"; // configure client
...
export default function Sports() {
  ...
  const categoryDisplay = (category) => {
    const displays = {
      sports: isUrdu ? "کھیل" : "Sports",
      cricket: isUrdu ? "کرکٹ" : "Cricket",
      football: isUrdu ? "فٹ بال" : "Football",
      tennis: isUrdu ? "ٹینس" : "Tennis",
    };
    return displays[category] || category;
  };

  useEffect(() => {
    const query = `*[_type == "news" && "sports" in categories]{
  ...
  return (
    <ScrollBasedAnimation direction="up" delay={0.2}>
      <section className={`max-w-7xl mx-auto px-8 py-16 md:py-24 ${isUrdu ? "rtl" : "ltr"}`}>
        <div className="flex items-center justify-between mb-12">
          <div className="flex items-center gap-4">
            <div className="w-1 h-8 bg-black" />
            <h2 className="text-3xl md:text-4xl font-bold tracking-tight uppercase">
              {isUrdu ? "کھیل خبریں" : "Sports"}
            </h2>
          </div>
          <a href="/sports-news" className="text-sm font-medium hover:underline transition">
