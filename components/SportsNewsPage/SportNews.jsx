...
export default function SportsNewsPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      const query = `*[_type == "news" && "sports" in categories] | order(publishedAt desc) {
  ...
  if (loading) {
    return (
      <main className="flex justify-center items-center h-screen text-gray-600">
        {isUrdu ? "لوڈ ہو رہا ہے..." : "Loading sports news..."}
      </main>
    );
  }
  ...
  if (!news.length) {
    return (
      <main className="flex justify-center items-center h-screen text-gray-600">
        {isUrdu ? "کھیل کی خبریں دستیاب نہیں ہیں۔" : "No sports news available."}
      </main>
    );
  }
      ...
      <ScrollBasedAnimation direction="up" delay={0.1}>
        <div className="flex items-center gap-4 mb-12">
          ...
          <h1 className="text-3xl md:text-5xl font-bold uppercase tracking-tight">
            {isUrdu ? "کھیل کی خبریں" : "Sports News"}
          </h1>
        </div>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-20">
          <div className="relative h-96 lg:h-[32rem] overflow-hidden">
            ...
            <div className="absolute top-4 left-4 px-3 py-1 border border-white bg-black/80">
              <span className="text-white text-xs font-semibold tracking-widest">
                {isUrdu
                  ? news[0].categories[0] // Assuming you keep category names in Urdu in Sanity for Urdu pages
                  : news[0].categories[0]}
              </span>
            </div>
          </div>
          ...
          <div className="flex flex-col justify-center">
            ...
            <p className="text-gray-700 text-base leading-relaxed">
              {isUrdu ? news[0].summary?.ur : news[0].summary?.en || "Read more about this sports update."}
            </p>
            <a
              href={`/${isUrdu ? "ur" : "en"}/sports-news/${news[0].slug.current}`}
        ...
        <section>
          <div className="flex items-center gap-4 mb-10">
            ...
            <h2 className="text-3xl md:text-4xl font-bold uppercase tracking-tight">
              {isUrdu ? "تازہ ترین کھیل کی خبریں" : "Latest Sports Updates"}
            </h2>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {paginatedNews.map((item, idx) => (
              ...
              >
                <a href={`/${isUrdu ? "ur" : "en"}/sports-news/${item.slug.current}`} className="block">
