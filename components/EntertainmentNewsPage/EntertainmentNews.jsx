...
export default function EntertainmentNewsPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      const query = `*[_type == "news" && "entertainment" in categories] | order(publishedAt desc) {
  ...
  if (loading) {
    return (
      <main className="flex justify-center items-center h-screen text-gray-600">
        {isUrdu ? "لوڈ ہو رہا ہے..." : "Loading entertainment news..."}
      </main>
    );
  }
  ...
  if (!news.length) {
    return (
      <main className="flex justify-center items-center h-screen text-gray-600">
        {isUrdu ? "انٹرٹینمنٹ کی خبریں دستیاب نہیں ہیں۔" : "No entertainment news available."}
      </main>
    );
  }
      ...
      <ScrollBasedAnimation direction="up" delay={0.1}>
        <div className="flex items-center gap-4 mb-12">
          ...
          <h1 className="text-3xl md:text-5xl font-bold uppercase tracking-tight">
            {isUrdu ? "انٹرٹینمنٹ خبریں" : "Entertainment News"}
          </h1>
        </div>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-20">
          ...
          <div className="flex flex-col justify-center">
            ...
            <p className="text-gray-700 text-base leading-relaxed">
              {isUrdu ? news[0].summary?.ur : news[0].summary?.en || "Read more about this entertainment news."}
            </p>
            <a
        ...
        <section>
          <div className="flex items-center gap-4 mb-10">
            ...
            <h2 className="text-3xl md:text-4xl font-bold uppercase tracking-tight">
              {isUrdu ? "تازہ ترین انٹرٹینمنٹ خبریں" : "Latest Entertainment Updates"}
            </h2>
          </div>
